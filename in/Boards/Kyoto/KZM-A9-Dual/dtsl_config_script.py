...
class DtslScript(DTSLv1):
        ...
        return [
            DTSLv1.tabSet("options", "Options", childOptions=[
                ...
                DTSLv1.tabPage("system", "System", childOptions=[
                   DTSLv1.booleanOption('useCtiSync', 'Enable CTI synchronization', defaultValue=False,
    ...
    def discoverDevices(self):
        ...
        # There are 3 candidates for outCTI - pretty sure it's the third one in the RVC file
        ...
        # There are two ETBs - we only need the second one
        etbDev0 = self.findDevice("CSETB")
        etbDev1 = self.findDevice("CSETB", etbDev0+1)
        self.ETB = ETBTraceCapture(self, etbDev1, "ETB")

        # There are two TPIUs - neither are used
        tpiuDev0 = self.findDevice("CSTPIU")
        self.tpiu0 = self.createTPIU(tpiuDev0, "TPIU_0")

        tpiuDev1 = self.findDevice("CSTPIU", tpiuDev0+1)
        self.tpiu1 = self.createTPIU(tpiuDev1, "TPIU_1")

        # There are two funnels - we only need the second funnel
        # To keep the rest of the code simple, the second funnel is called funnel0

        # Funnel 0
        funnelDev0 = self.findDevice("CSTFunnel")
        funnelDev0 = self.findDevice("CSTFunnel", funnelDev0+1)
    ...
    def setupETBTrace(self):
        ...
        self.addManagedTraceDevices("ETB", [ self.funnel0, self.tpiu0, self.tpiu1, self.outCTI, self.ETB ])
