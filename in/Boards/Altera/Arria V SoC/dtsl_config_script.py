...
from com.arm.debug.dtsl.components import STMRegisters
...
class CTICore(Device):
    ...
    def configureCTIs(self):
        # Not always safe to do this, depends on loaded SOF. Defer it until after post-connect check
...
''' Overload PTM Trace Source with a version that will report the ability to deliver timestamps.
    Needed to correct problems with target's ETMCCER register
'''
...
The system clocks are typically set up by running the Altera preloader script'''
        ...
        return DTSLv1.tabPage('xtrig', 'Cross Trigger', childOptions=[
            DTSLv1.booleanOption('xtrigFPGA0', 'Enable FPGA -> HPS Cross Trigger (Cortex-A9_0) ', defaultValue=False, isDynamic=True,
                setter=DtslScript.setXTrigFPGAEnabled0, description="Trigger events in the FPGA can halt only Cortex-A9_0 core"),
            DTSLv1.booleanOption('xtrigFPGA1', 'Enable FPGA -> HPS Cross Trigger (Cortex-A9_1)', defaultValue=False, isDynamic=True,
                setter=DtslScript.setXTrigFPGAEnabled1, description="Trigger events in the FPGA can halt only Cortex-A9_1 core"),
            ...
            DTSLv1.optionGroup('access', 'Cross Trigger initialization', childOptions=[
                DTSLv1.infoElement('info', INIT_INFO),
    ...
    def discoverDevices(self):
        ...
        # ETR and ETF
        # The ETF comes first (0x80001000) and the ETR next (0x80006000)
        ...
        self.STM = STMImpl(self, stmDev, streamID, "STM")
        streamID += 1
    ...
    def setupETRTrace(self):
        ...
        self.addManagedTraceDevices("ETR", [ self.funnel0, self.outCTI, self.tpiu, self.ETR, self.ETF ])
    ...
    def setupDSTREAMTrace(self, portWidth):
        ...
        # automatically handle connection/disconnection to trace components
        self.addManagedTraceDevices("DSTREAM", [ self.funnel0, self.outCTI, self.tpiu, self.DSTREAM, self.ETF ])
    ...
    def setXTrigFPGAEnabled0(self, enabled):
        self.cortexA9cores[0].setCrossTrigFPGA(enabled)

    def setXTrigFPGAEnabled1(self, enabled):
        self.cortexA9cores[1].setCrossTrigFPGA(enabled)
    ...
    def registerFilters(self, core):
        ...
        core.registerAddressFilters(
            [AHBMemAPAccessor("AHB", self.AHB, "AHB bus accessed via AP_0"),
    ...
    def postConnect(self):
        ...
        freq = self.getOptionValue("options.traceBuffer.timestampFrequency")
...
class DSTREAM_DtslScript(DtslScript):
    ...
    def getOptionTraceCaptureTabPage():
        return DTSLv1.tabPage("traceBuffer", "Trace Capture", childOptions=[
            ...
            DTSLv1.infoElement(
                ...
                childOptions=[
                    DTSLv1.enumOption('tpiuPortWidth', 'TPIU Port Width', defaultValue="16",
                        values = [("%d" % x, "%d bit" % x) for x in xrange(1,17)], isDynamic=False)
                ]
            )
        )
...
class DtslScript_DSTREAM_ST_Family(DtslScript):
    ...
    def setupDSTREAMTrace(self, portWidth):
        ...
        # automatically handle connection/disconnection to trace components
        self.addManagedTraceDevices(self.getOptionValue("options.traceBuffer.traceCapture"), [ self.funnel0, self.outCTI, self.tpiu, self.DSTREAM, self.ETF ])
    ...
    def getOptionTraceCaptureTabPage():
        return DTSLv1.tabPage("traceBuffer", "Trace Capture", childOptions=[
            ...
            DTSLv1.enumOption('traceCapture', 'Trace capture method', defaultValue="none",
                values = [("none", "None"), ("ETR", "System Memory Trace Buffer (ETR)"), ("ETF", "On Chip Trace Buffer (ETF)")],
                setter=DtslScript.setTraceCaptureMethod),
            DTSLv1.integerOption('timestampFrequency', 'Timestamp frequency', defaultValue=25000000, isDynamic=False, description="This value will be used to set the Counter Base Frequency ID Register of the Timestamp generator.\nIt represents the number of ticks per second and is used to translate the timestamp value reported into a number of seconds.\nNote that changing this value may not result in a change in the observed frequency."),
        ])
...
class CheckedTPIU(CSTPIU):
    ...
    def setMemAccessor(self, memAcc):
        self.memAccessor = memAcc
    ...
    def confirmTraceSetupDone(self):
        # $sysmgr_romcodegrp_initswstate
        if self.confirmValue(0xffd080c8, [0x49535756]) == False:
            return False

        # $sysmgr_pinmuxgrp_GENERALIO0-8
        expected = [ 0x3, 0x3, 0x3, 0x3, 0x3, 0x3, 0x3, 0x3, 0x3]
        if self.confirmValue(0xffd08480, expected) == False:
            return False

        return True

    def confirmValue(self, addr, expected):
        length = 4*len(expected)
        buffer = zeros(length, 'b')
        try:
            self.memAccessor.readMem(addr, length, buffer)
            actual = [ unpack('<I', buffer[i:i+4])[0] for i in range(0, len(buffer), 4) ]
        except DTSLException:
            return False
        for e, a in zip(expected, actual):
            if (a != e):
                #print "Expected %08x but read %08x" % (e, a)
                return False
...
class USBBlaster_DtslScript(NoDSTREAMTrace_DtslScript):
    @staticmethod
    def getOptionList():
        return NoDSTREAMTrace_DtslScript.getOptionList()
