...
class DtslScript(DTSLv1):
    ...
    def discoverDevices(self):
        ...
        for i in range(0, NUM_CORES_CORTEX_A9):
            ...
            ''' Core 0 will wake up any other cores on system reset '''
            if i == 0:
                dev = WakingDevice(self, cortexA9coreDev, "Cortex-A9_%d" % i)
            else:
...
class WakingDevice(Device):
    ''' This class extends the core device to call back into the connection when
    a system reset is performed.  This callback is used to re-awaken core 1 if
    required. '''
    def __init__(self, root, device, name):
        Device.__init__(self, root, device, name)
        self.parent = root

    def systemReset(self, resetType):
        Device.systemReset(self, resetType)
        self.parent.conditionallyWakeCore1()
