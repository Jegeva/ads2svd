...
class DtslScript(DTSLv1):
    ...
    def discoverDevices(self):
        ...
        self.cores = dict()
        self.createModelCore("css.scp.armcortexm3ct", "ARM_Cortex-M3", "Cortex-M3")
        self.createModelCore("css.cluster0.cpu0", "ARM_Cortex-A55_0", "Cortex-A55")
        self.createModelCore("css.cluster0.cpu1", "ARM_Cortex-A55_1", "Cortex-A55")
        self.createModelCore("css.cluster0.cpu2", "ARM_Cortex-A55_2", "Cortex-A55")
        self.createModelCore("css.cluster0.cpu3", "ARM_Cortex-A55_3", "Cortex-A55")
        self.createModelCore("css.cluster0.cpu4", "ARM_Cortex-A75_0", "Cortex-A75")
        self.createModelCore("css.cluster0.cpu5", "ARM_Cortex-A75_1", "Cortex-A75")
        self.createModelCore("css.cluster0.cpu6", "ARM_Cortex-A75_2", "Cortex-A75")
        self.createModelCore("css.cluster0.cpu7", "ARM_Cortex-A75_3", "Cortex-A75")
        ...
        self.cluster0cores.append(self.cores["css.cluster0.cpu4"])
        self.cluster0cores.append(self.cores["css.cluster0.cpu5"])
        self.cluster0cores.append(self.cores["css.cluster0.cpu6"])
        self.cluster0cores.append(self.cores["css.cluster0.cpu7"])
    ...
    def setupModelTrace(self):
        ...
        if "css.cluster0.cpu0" in self.cores.keys():
            fmtSource =  FMTraceSource(self, DeviceId, StreamId, "FMT_0")
        ...
        if "css.cluster0.cpu4" in self.cores.keys():
            fmtSource =  FMTraceSource(self, DeviceId+4, StreamId+4, "FMT_4")
            self.traceSources.append(fmtSource)
            self.tracecapture.addTraceSource(fmtSource, self.cores["css.cluster0.cpu4"].getID())
            fmtSource.setEnabled(True)
        
        if "css.cluster0.cpu5" in self.cores.keys():
            fmtSource =  FMTraceSource(self, DeviceId+5, StreamId+5, "FMT_5")
            self.traceSources.append(fmtSource)
            self.tracecapture.addTraceSource(fmtSource, self.cores["css.cluster0.cpu5"].getID())
            fmtSource.setEnabled(True)
        
        if "css.cluster0.cpu6" in self.cores.keys():
            fmtSource =  FMTraceSource(self, DeviceId+6, StreamId+6, "FMT_6")
            self.traceSources.append(fmtSource)
            self.tracecapture.addTraceSource(fmtSource, self.cores["css.cluster0.cpu6"].getID())
            fmtSource.setEnabled(True)
        
        if "css.cluster0.cpu7" in self.cores.keys():
            fmtSource =  FMTraceSource(self, DeviceId+7, StreamId+7, "FMT_7")
            self.traceSources.append(fmtSource)
            self.tracecapture.addTraceSource(fmtSource, self.cores["css.cluster0.cpu7"].getID())
            fmtSource.setEnabled(True)
        
        if "css.scp.armcortexm3ct" in self.cores.keys():
            fmtSource =  FMTraceSource(self, DeviceId+8, StreamId+8, "FMT_8")
            self.traceSources.append(fmtSource)
            self.tracecapture.addTraceSource(fmtSource, self.cores["css.scp.armcortexm3ct"].getID())
            fmtSource.setEnabled(True)
    ...
    def setupCadiSMP(self):
        #cores = [ DeviceCluster("big", self.cluster0cores), DeviceCluster("LITTLE", self.cluster1cores) ]
        smp = CadiSyncSMPDevice(self, "ARM Cortex-A75x4 Cortex-A55x4 Cluster 0", self.cluster0cores)
