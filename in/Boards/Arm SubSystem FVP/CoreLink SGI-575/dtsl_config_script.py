...
class DtslScript(DTSLv1):
    ...
    def discoverDevices(self):
        ...
        self.cores = dict()
        self.createModelCore("css.scp.armcortexm7ct", "ARM_Cortex-M7_1")
        self.createModelCore("css.mcp.armcortexm7ct", "ARM_Cortex-M7_0")
        self.createModelCore("css.cluster1.cpu0", "Cortex-A75_4")
        self.createModelCore("css.cluster1.cpu1", "Cortex-A75_5")
        self.createModelCore("css.cluster1.cpu2", "Cortex-A75_6")
        self.createModelCore("css.cluster1.cpu3", "Cortex-A75_7")
        self.createModelCore("css.cluster0.cpu0", "Cortex-A75_0")
        self.createModelCore("css.cluster0.cpu1", "Cortex-A75_1")
        self.createModelCore("css.cluster0.cpu2", "Cortex-A75_2")
        self.createModelCore("css.cluster0.cpu3", "Cortex-A75_3")
        
        self.cluster1cores = []
        self.addModelCoreToCluster(self.cluster1cores, "css.cluster1.cpu0")
        self.addModelCoreToCluster(self.cluster1cores, "css.cluster1.cpu1")
        self.addModelCoreToCluster(self.cluster1cores, "css.cluster1.cpu2")
        self.addModelCoreToCluster(self.cluster1cores, "css.cluster1.cpu3")
        
        self.cluster0cores = []
        self.addModelCoreToCluster(self.cluster0cores, "css.cluster0.cpu0")
        self.addModelCoreToCluster(self.cluster0cores, "css.cluster0.cpu1")
        self.addModelCoreToCluster(self.cluster0cores, "css.cluster0.cpu2")
        self.addModelCoreToCluster(self.cluster0cores, "css.cluster0.cpu3")
    ...
    def setupModelTrace(self):
        ...
        if "css.cluster0.cpu0" in self.cores.keys():
        ...
        if "css.cluster0.cpu1" in self.cores.keys():
        ...
        if "css.cluster0.cpu2" in self.cores.keys():
            fmtSource =  FMTraceSource(self, DeviceId+2, StreamId+2, "FMT_2")
            self.traceSources.append(fmtSource)
            self.tracecapture.addTraceSource(fmtSource, self.cores["css.cluster0.cpu2"].getID())
            fmtSource.setEnabled(True)
        
        if "css.cluster0.cpu3" in self.cores.keys():
            fmtSource =  FMTraceSource(self, DeviceId+3, StreamId+3, "FMT_3")
            self.traceSources.append(fmtSource)
            self.tracecapture.addTraceSource(fmtSource, self.cores["css.cluster0.cpu3"].getID())
            fmtSource.setEnabled(True)
        
        if "css.cluster1.cpu0" in self.cores.keys():
        ...
        if "css.cluster1.cpu1" in self.cores.keys():
        ...
        if "css.cluster1.cpu2" in self.cores.keys():
            fmtSource =  FMTraceSource(self, DeviceId+6, StreamId+6, "FMT_6")
            self.traceSources.append(fmtSource)
            self.tracecapture.addTraceSource(fmtSource, self.cores["css.cluster1.cpu2"].getID())
            fmtSource.setEnabled(True)
        
        if "css.cluster1.cpu3" in self.cores.keys():
            fmtSource =  FMTraceSource(self, DeviceId+7, StreamId+7, "FMT_7")
            self.traceSources.append(fmtSource)
            self.tracecapture.addTraceSource(fmtSource, self.cores["css.cluster1.cpu3"].getID())
            fmtSource.setEnabled(True)
        
        if "css.mcp.armcortexm7ct" in self.cores.keys():
        ...
        if "css.scp.armcortexm7ct" in self.cores.keys():
    ...
    def setupCadiSyncSMP(self):
        ...
        # cluster0 SMP
        smp = CadiSyncSMPDevice(self, "Cortex-A75x4 SMP Cluster 0", self.cluster0cores)
        self.addDeviceInterface(smp)
        
        # cluster1 SMP
        smp = CadiSyncSMPDevice(self, "Cortex-A75x4 SMP Cluster 1", self.cluster1cores)
        self.addDeviceInterface(smp)
        
        # MULTI CLUSTER SMP
        clusters = [ DeviceCluster("cluster0", self.cluster0cores), DeviceCluster("cluster1", self.cluster1cores) ]
        smp = CadiSyncSMPDevice(self, "Cortex-A75x8 Multi-Cluster SMP", clusters)
        self.addDeviceInterface(smp)
