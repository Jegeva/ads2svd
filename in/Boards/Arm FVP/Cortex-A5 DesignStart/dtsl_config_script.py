...
class DtslScript(DTSLv1):
    ...
    def discoverDevices(self):
        ...
        self.cores = dict()
        self.createModelCore("css.cluster.cpu0", "Cortex-A5_0", "Cortex-A5")
        self.createModelCore("css.cluster.cpu1", "Cortex-A5_1", "Cortex-A5")
        self.createModelCore("css.cluster.cpu2", "Cortex-A5_2", "Cortex-A5")
        self.createModelCore("css.cluster.cpu3", "Cortex-A5_3", "Cortex-A5")

        self.cluster0cores = []
        self.addModelCoreToCluster(self.cluster0cores, "css.cluster.cpu0")
        self.addModelCoreToCluster(self.cluster0cores, "css.cluster.cpu1")
        self.addModelCoreToCluster(self.cluster0cores, "css.cluster.cpu2")
        self.addModelCoreToCluster(self.cluster0cores, "css.cluster.cpu3")


        self.caches = dict()
        self.createModelCache("css.cluster.cpu0.l1icache", "l1icache_0")
        self.createModelCache("css.cluster.cpu0.l1dcache", "l1dcache_0")
        self.createModelCache("css.cluster.cpu1.l1icache", "l1icache_1")
        self.createModelCache("css.cluster.cpu1.l1dcache", "l1dcache_1")
        self.createModelCache("css.cluster.cpu2.l1icache", "l1icache_2")
        self.createModelCache("css.cluster.cpu2.l1dcache", "l1dcache_2")
        self.createModelCache("css.cluster.cpu3.l1icache", "l1icache_3")
        self.createModelCache("css.cluster.cpu3.l1dcache", "l1dcache_3")
        self.addPVCache("css.cluster.cpu0", "css.cluster.cpu0.l1icache", "css.cluster.cpu0.l1dcache")
        self.addPVCache("css.cluster.cpu1", "css.cluster.cpu1.l1icache", "css.cluster.cpu1.l1dcache")
        self.addPVCache("css.cluster.cpu2", "css.cluster.cpu2.l1icache", "css.cluster.cpu2.l1dcache")
        self.addPVCache("css.cluster.cpu3", "css.cluster.cpu3.l1icache", "css.cluster.cpu3.l1dcache")
    ...
    def setupModelTrace(self):
        ...
        if "css.cluster.cpu0" in self.cores.keys():
            fmtSource =  FMTraceSource(self, DeviceId+0, StreamId+0, "FMT_0")
            self.traceSources.append(fmtSource)
            self.tracecapture.addTraceSource(fmtSource, self.cores["css.cluster.cpu0"].getID())
            fmtSource.setEnabled(True)

        if "css.cluster.cpu1" in self.cores.keys():
            fmtSource =  FMTraceSource(self, DeviceId+1, StreamId+1, "FMT_1")
            self.traceSources.append(fmtSource)
            self.tracecapture.addTraceSource(fmtSource, self.cores["css.cluster.cpu1"].getID())
            fmtSource.setEnabled(True)

        if "css.cluster.cpu2" in self.cores.keys():
            fmtSource =  FMTraceSource(self, DeviceId+2, StreamId+2, "FMT_2")
            self.traceSources.append(fmtSource)
            self.tracecapture.addTraceSource(fmtSource, self.cores["css.cluster.cpu2"].getID())
            fmtSource.setEnabled(True)

        if "css.cluster.cpu3" in self.cores.keys():
            fmtSource =  FMTraceSource(self, DeviceId+3, StreamId+3, "FMT_3")
            self.traceSources.append(fmtSource)
            self.tracecapture.addTraceSource(fmtSource, self.cores["css.cluster.cpu3"].getID())
            fmtSource.setEnabled(True)
    ...
    def setupCadiSyncSMP(self):
        ...
        # cluster0 SMP
        smp = CadiSyncSMPDevice(self, "Cortex-A5x4 SMP", self.cluster0cores)
