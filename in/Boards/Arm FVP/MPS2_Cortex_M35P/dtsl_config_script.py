...
class DtslScript(DTSLv1):
    ...
    def discoverDevices(self):
        ...
        self.cores = dict()
        self.createModelCore("cpu0", "ARM_Cortex-M35P_0", "Cortex-M35P")
        self.createModelCore("cpu1", "ARM_Cortex-M35P_1", "Cortex-M35P")
    ...
    def setupModelTrace(self):
        ...
        if "cpu0" in self.cores.keys():
            fmtSource =  FMTraceSource(self, DeviceId+0, StreamId+0, "FMT_0")
            self.traceSources.append(fmtSource)
            self.tracecapture.addTraceSource(fmtSource, self.cores["cpu0"].getID())
            fmtSource.setEnabled(True)
        
        if "cpu1" in self.cores.keys():
            fmtSource =  FMTraceSource(self, DeviceId+1, StreamId+1, "FMT_1")
            self.traceSources.append(fmtSource)
            self.tracecapture.addTraceSource(fmtSource, self.cores["cpu1"].getID())
            fmtSource.setEnabled(True)
