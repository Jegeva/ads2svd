...
class DtslScript(DTSLv1):
    ...
    def setupETBTrace(self):
        ...
        self.addManagedTraceDevices("ETB", [ self.funnel0, self.tpiu, self.ETB ])
    ...
    def setupDSTREAMTrace(self, portwidth):
        ...
        # automatically handle connection/disconnection to trace components
        self.addManagedTraceDevices("DSTREAM", [ self.funnel0, self.tpiu, self.DSTREAM ])

    def setupRDDISyncSMP(self):
        '''Create SMP device using non-CTI synchronization'''
    ...
    def optionValuesChanged(self):
        ...
        for i in range(0, NUM_CORES_CORTEX_A9):
            ...
            self.setTriggerGeneratesDBGRQ(self.PTMs[i], self.getOptionValue("options.coreTrace.cortexA9coreTrace.triggerhalt"))
            self.setContextIDEnabled(self.PTMs[i],
                                     self.getOptionValue("options.coreTrace.cortexA9coreTrace.contextIDs"),
                                     self.getOptionValue("options.coreTrace.cortexA9coreTrace.contextIDs.contextIDsSize"))
