...
class TraceRangeOptions:
    def __init__(self, coreTraceName = None, dtsl = None):
        if coreTraceName == None:
            ...
            self.traceRangeEnable = dtsl.getOptionValue("options.%s.traceRange" % coreTraceName)
            self.traceRangeStart = dtsl.getOptionValue("options.%s.traceRange.start" % coreTraceName)
            self.traceRangeEnd = dtsl.getOptionValue("options.%s.traceRange.end" % coreTraceName)
...
class DtslScript(DTSLv1):
    ...
    def optionValuesChanged(self):
        ...
        self.setInternalTraceRange(self.traceRangeOptions[CORTEX_A9_TRACE_OPTIONS], TraceRangeOptions("cortexA9.coreTrace", self), self.PTMs[ptmStartIndex:ptmEndIndex])
    ...
    def getOptionList():
        return [
            DTSLv1.tabSet("options", "Options", childOptions=[
                ...
                DTSLv1.tabPage("cortexA9", "Cortex-A9", childOptions=[
                    DTSLv1.booleanOption('coreTrace', 'Enable Cortex-A9 core trace', defaultValue=False,
                        childOptions =
                            ...
                            # Pull in common options for PTMs (cycle accurate etc)
                            PTMTraceSource.defaultOptions(DtslScript_RVI.getPTMs) +
                            [ # Trace range selection (e.g. for linux kernel)
                            DTSLv1.booleanOption('traceRange', 'Trace capture range',
                                description=TRACE_RANGE_DESCRIPTION,
                                defaultValue = False,
                ...
                DTSLv1.tabPage("itm", "ITM", childOptions=[
                    DTSLv1.booleanOption('itm', 'Enable ITM trace', defaultValue=False,
                        setter=DtslScript_RVI.setITMEnabled),
                ]),
            ]),
        ]
