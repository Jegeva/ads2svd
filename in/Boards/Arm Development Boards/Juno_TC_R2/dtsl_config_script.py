...
class ResetHookedA72Core(a72_rams.A72CoreDevice):
    def __init__(self, config, id, name):
        a72_rams.A72CoreDevice.__init__(self, config, id, name)
        self.parent = config
...
class DtslScript(DTSLv1):
        ...
        return DTSLv1.tabPage("traceBuffer", "Trace Buffer", childOptions=[
                    DTSLv1.enumOption(
                        ...
                        values=[("none", "None"),
                                ...
                                ("ETF0", "On Chip Trace Buffer (ETF0 - A72/A53 Core Trace Only)"),
                                ("ETF1", "On Chip Trace Buffer (ETF1 - STM/SCP/Profiler Trace Only)"),
                                ("DSTREAM", "DSTREAM 4GB Trace Buffer")],
                        setter=DtslScript.setTraceCaptureMethod)
        ])

    @staticmethod
    def getOptionCortexA72TabPage():
        return DTSLv1.tabPage("cortexA72", "Cortex-A72", childOptions=[
                DTSLv1.booleanOption(
                    'cortexA72coreTrace',
                    'Enable Cortex-A72 core trace',
                    defaultValue=False,
                    childOptions=[
                        DTSLv1.booleanOption(
                            'Cortex_A72_%d' % c,
                            "Enable Cortex-A72 %d trace" % c,
                            defaultValue=True)
                            for c in range(0, NUM_A72_CORES)] + [
                        DTSLv1.booleanOption(
                            'triggerhalt',
                        ...
                        DTSLv1.booleanOption(
                            ...
                            description="Controls the output of context ID "
                                        ...
                                        defaultValue=True)] + [
                        ETMv4TraceSource.cycleAccurateOption(DtslScript.getA72ETMs)] + [
    ...
    def getOptionETRandETFTabPage():
        return DTSLv1.tabPage("ETR", "ETR/ETF", childOptions=[
            DTSLv1.booleanOption(
                'syncBuffer',
                'Halt Cortex-A72/A53 SMP cluster(s) when ETR/ETF buffer full',
                defaultValue=False,
                isDynamic=True),
            DTSLv1.booleanOption(
                ...
                childOptions=[
                    DTSLv1.integerOption('start', 'Start address',
                        description='Start address of the on-chip buffer.'
                                    '\nThe start address must indicate the start of a contiguous block of available memory,\n '
                                    'or a configured scatter-gather table if using scatter-gather mode.',
        ...
        return DTSLv1.tabPage(
                ...
                childOptions=[
                    ...
                    DTSLv1.optionGroup('monitors',
                        ...
                        DTSLv1.booleanOption(
                            "mon_0",
                            "Monitor 0 (ABM attached to Cortex-A72)",
                            defaultValue=False),
                        DTSLv1.booleanOption(
                            "mon_1",
                ...
                childOptions=[
                    ...
                    DTSLv1.infoElement(
                        ...
                        childOptions=[
                            ...
                            DTSLv1.booleanOption(
                                name="cortex-a72_0",
                                displayName="Cortex-A72_0 enters debug state",
                            ...
                            DTSLv1.booleanOption(
                                name="cortex-a72_1",
                                displayName="Cortex-A72_1 enters debug state",
                                description='Debug state is entered when the '
                                            'core hits a breakpoint or when '
                                            'the use manually halts it',
                                defaultValue=False,
    ...
    def getOptionList():
        return [
            DTSLv1.tabSet("options", "Options", childOptions=[
                ...
                DtslScript.getOptionCortexA72TabPage(),
    ...
    def __init__(self, root):
        ...
        for core in self.cortexA72cores:
            a72_rams.registerInternalRAMs(core)
        for core in self.cortexA53cores:
            a53_rams.registerInternalRAMs(core)
        cores = self.cortexA53cores + self.cortexA72cores + self.cortexM3cores
        for core in cores:
    ...
    def discoverDevices(self):
        ...
        self.sysCtrl = ConnectableDevice(self,
                                         self.findDevice("JunoR2SystemControl"),
                                         "JunoR2SystemControl")
        ...
        self.sysCTI1 = CSCTI(self, sysCTIDev, "CTI_sys_1")

        # The system CTI for profiler, generic counter, watchdog timer
        sysCTIDev = self.findDevice("CSCTI", sysCTIDev + 1)
        self.sysCTI2 = CSCTI(self, sysCTIDev, "CTI_sys_2")

        ##############################################################################################

        # map cores/ETMs to associated CTIs
        self.cortexA72ctiMap = {}
        ...
        for i in range(0, NUM_A72_CORES):
            # create core
            coreDev = self.findDevice("Cortex-A72", coreDev + 1)
            dev = ResetHookedA72Core(self, coreDev, "Cortex-A72_%d" % i)
            deviceInfo = DeviceInfo("core", "Cortex-A72")
            dev.setDeviceInfo(deviceInfo)
            self.cortexA72cores.append(dev)
            ...
            self.cortexA72ctiMap[dev] = coreCTI
        ...
        self.ETF0 = CSTMC(self, tmcDev, "ETF0")

        # Reserve this TMC for use as an ETR
        self.etrTmcDev = self.findDevice("CSTMC", tmcDev + 1)

        tmcDev = self.findDevice("CSTMC", self.etrTmcDev + 1)
        self.ETF1 = CSTMC(self, tmcDev, "ETF1")
        ...
        self.funnelApps = self.createFunnel(funnelDev, "Funnel_apps")
        self.funnelApps.setPortEnabled(0)  # A72 trace port always enabled here - controlled in cluster funnel
        self.funnelApps.setPortEnabled(1)  # A53 trace port always enabled here - controlled in cluster funnel

        # SCP
        funnelDev = self.findDevice("CSTFunnel", funnelDev + 1)
        self.funnelSCP = self.createFunnel(funnelDev, "Funnel_SCP")
        self.funnelSCP.setPortEnabled(1)  # SCP trace port always enabled here - controlled in cluster funnel
        ...
        #cluster funnels
        self.funnelMaster.setPortEnabled(0)  # Apps Trace
        self.funnelMaster.setPortEnabled(1)  # SCP/STM/Profiler trace
        ####################################################################################


        #Individual cluster funnels
        funnelDev = self.findDevice("CSTFunnel", funnelDev + 1)
        self.funnelA72 = self.createFunnel(funnelDev, "Funnel_A72")
        funnelDev = self.findDevice("CSTFunnel", funnelDev + 1)
    ...
    def createETFTrace0(self):
        self.etfTrace0 = TMCETBTraceCapture(self, self.ETF0, "ETF0")

    def createETFTrace1(self):
        self.etfTrace1 = TMCETBTraceCapture(self, self.ETF1, "ETF1")
    ...
    def setupDSTREAMTrace(self, portWidth):
        ...
        self.DSTREAM.setTraceComponentOrder(
                         [self.funnelA72, self.funnelA53, self.funnelM3, self.funnelApps, self.funnelSCP,
                          self.ETF0, self.ETF1, self.funnelMaster, self.tpiu])
        ...
        self.addManagedTraceDevices("DSTREAM",
                 [self.funnelMaster, self.funnelApps, self.funnelSCP, self.funnelA72, self.funnelA53,
                  self.funnelM3, self.sysCTI0, self.sysCTI2, self.tpiu,
                  self.DSTREAM, self.ETF0, self.ETF1])
    ...
    def setupETRTrace(self):
        ...
        self.ETR.setTraceComponentOrder([self.funnelM3, self.funnelA72, self.funnelA53,
                                         self.funnelApps, self.funnelSCP, self.ETF0, self.ETF1,
                                         self.funnelMaster,
                                         ])
        self.addTraceCaptureInterface(self.ETR)

        # automatically handle connection/disconnection to trace components
        self.addManagedTraceDevices("ETR", [self.funnelMaster, self.funnelApps, self.funnelSCP, self.funnelA72,
                                            self.funnelA53, self.funnelM3, self.sysCTI0, self.tpiu,
                                            self.ETR, self.ETF0, self.ETF1])

    def setupETFTrace0(self):
        self.etfTrace0.setFormatterMode(FormatterMode.CONTINUOUS)

        # register other trace components with ETF and register ETF with config
        self.etfTrace0.setTraceComponentOrder([self.funnelM3, self.funnelA72,
                          self.funnelA53, self.funnelApps, self.ETF0])

        self.addTraceCaptureInterface(self.etfTrace0)

        # automatically handle connection/disconnection to trace components
        self.addManagedTraceDevices(
            "ETF0", [self.funnelM3, self.funnelA72, self.funnelA53, self.funnelApps,
                     self.sysCTI0, self.tpiu, self.etfTrace0])

    def setupETFTrace1(self):
        self.etfTrace1.setFormatterMode(FormatterMode.CONTINUOUS)

        # register other trace components with ETF and register ETF with config
        self.etfTrace1.setTraceComponentOrder([self.funnelM3, self.funnelSCP, self.ETF1])

        self.addTraceCaptureInterface(self.etfTrace1)

        # automatically handle connection/disconnection to trace components
        self.addManagedTraceDevices(
            "ETF1", [self.funnelM3, self.funnelSCP,
                     self.sysCTI2, self.tpiu, self.etfTrace1])

    def setupCTISyncSMP(self):
        # Cortex-A72 CTI SMP setup
        ctiInfo = {}
        for c in self.cortexA72cores:
            ...
            ctiInfo[c] = CTISyncSMPDevice.DeviceCTIInfo(
                            self.cortexA72ctiMap[c],
                            CTISyncSMPDevice.DeviceCTIInfo.NONE, 1, 0, 0)

        smpA72 = CTISyncSMPDevice(self,
                                  "Cortex-A72 SMP",
                                  self.cortexA72cores,
                                  ctiInfo,
                                  CTM_CHAN_SYNC_START,
                                  CTM_CHAN_SYNC_STOP)
        self.registerFilters(smpA72)
        self.addDeviceInterface(smpA72)
    ...
    def setupBigLittle(self):
        ...
        big = DeviceCluster("big", self.cortexA72cores)
    ...
    def setTraceCaptureMethod(self, method):
        ...
        if method == "none":
            ...
            self.ETF0.setMode(CSTMC.Mode.ETF)
            self.ETF1.setMode(CSTMC.Mode.ETF)
            self.setETFTrace0Enabled(False)
            self.setETFTrace1Enabled(False)
            ...
            self.ETF0.setMode(CSTMC.Mode.ETB)
            self.ETF1.setMode(CSTMC.Mode.ETF)
            self.setDSTREAMTraceEnabled(False)
            self.setETRTraceEnabled(False)
            self.setETFTrace0Enabled(True)
            self.setETFTrace1Enabled(False)

        elif method == "ETF1":
            self.ETF0.setMode(CSTMC.Mode.ETF)
            self.ETF1.setMode(CSTMC.Mode.ETB)
            self.setDSTREAMTraceEnabled(False)
            self.setETRTraceEnabled(False)
            self.setETFTrace1Enabled(True)
            self.setETFTrace0Enabled(False)
    ...
    def setETFTrace0Enabled(self, enabled):
        '''Enable/disable ETF trace capture'''
        self.etfTrace0Enabled = enabled
        self.enableCTIsForSink("etfTrace0", enabled)

    def setETFTrace1Enabled(self, enabled):
        '''Enable/disable ETF trace capture'''
        self.etfTrace1Enabled = enabled
        self.enableCTIsForSink("etfTrace1", enabled)
    ...
    def getCTIForSink(self, sink):
        ...
        if sink == "TPIU":
            ...
            return (self.sysCTI2, 1, CTM_CHAN_TRACE_TRIGGER)
    ...
    ''' Cores 0 and 1 (A72s) are on the A72 funnel ports 0 & 1
        Cores 2, 3, 4 & 5 (A53s) are on the A53 funnel ports 0, 1, 2 & 3
        Core 6 (M3) is on M3 funnel port 1'''
    def getFunnelPortForCore(self, core):
        ''' Funnel port-to-core mapping can be customized here'''
        port = core
        if core >= NUM_A72_CORES:
            port = core - NUM_A72_CORES
        if core >= (NUM_A72_CORES + NUM_A53_CORES):
            port = (core - (NUM_A72_CORES + NUM_A53_CORES)) + 1
        return port

    def getFunnelForSource(self, source):
        funnelMap = {self.STM: self.funnelSCP}
        funnelMap[self.ITM] = self.funnelM3
        funnelMap[self.sysProf] = self.funnelSCP
        totalCores = NUM_A53_CORES + NUM_A72_CORES + NUM_M3_CORES
        for i in range(0, totalCores):
            funnel = self.funnelA72
            if i >= NUM_A72_CORES:
                funnel = self.funnelA53
            if i >= (NUM_A53_CORES + NUM_A72_CORES):
    ...
    def getFunnelPortForSource(self, source):
        ...
        for i in range(0, NUM_A72_CORES + NUM_A53_CORES + NUM_M3_CORES):
    ...
    def registerTraceSources(self, traceCapture):
        ...
        for coreIndex in range(0, NUM_M3_CORES):
            etmIndex = NUM_A72_CORES + NUM_A53_CORES + coreIndex
            if self.ETMs[etmIndex].isEnabled():
                self.registerCoreTraceSource(traceCapture,
    ...
    def setupStopClockTriggers(self, obase):
        ...
        if self.getOptionValue(obase):
            ...
            if self.getOptionValue(triggerBase + ".cortex-a72_0"):
                cfgreg = cfgreg | (SCDBG_MASTER_EN |
                                   SCDBG_TRIG_DBGACK_ATL0)
            else:
                cfgreg = cfgreg & ~SCDBG_TRIG_DBGACK_ATL0
            if self.getOptionValue(triggerBase + ".cortex-a72_1"):
                cfgreg = cfgreg | (SCDBG_MASTER_EN |
    ...
    def setInitialOptions(self, obase):
        ...
        if self.dstreamTraceEnabled:
            ...
            self.createETR()
            self.setupETRTrace()
        elif self.etfTrace0Enabled:
            self.createETFTrace0()
            self.setupETFTrace0()
        elif self.etfTrace1Enabled:
            self.createETFTrace1()
            self.setupETFTrace1()

        obase = "options.cortexA72.cortexA72coreTrace"
        coreTraceEnabled = self.getOptionValue(obase)
        for i in range(0, NUM_A72_CORES):
            traceThisCore = self.getOptionValue(obase + ".Cortex_A72_%d" % i)
            enableSource = coreTraceEnabled and traceThisCore
            self.setTraceSourceEnabled(self.ETMs[i], enableSource)
            self.setInternalTraceRange(self.ETMs[i], "cortexA72")
        ...
        for i in range(0, NUM_M3_CORES):
            ...
            curIndex = i + NUM_A72_CORES + NUM_A53_CORES
            enableSource = coreTraceEnabled and traceThisCore
            self.setTraceSourceEnabled(self.ETMs[curIndex], enableSource)
            val = self.getOptionValue(obase + ".timestamp")
            self.setTimestampingEnabled(self.ETMs[curIndex], val)
        ...
        if self.etrTraceEnabled:
            ...
            self.registerTraceSources(self.etfTrace0)
        elif self.etfTrace1Enabled:
            self.registerTraceSources(self.etfTrace1)
    ...
    def traceStop(self, traceCapture):
        if self.getOptionValue("options.profiler.enable"):
            self.sysProf.traceStop(None)
            self.funnelSCP.setPortDisabled(SYS_PROFILER_FUNNEL_PORT)
        DTSLv1.traceStop(self, traceCapture)
    ...
    def traceStart(self, traceCapture):
        ...
        if self.getOptionValue("options.profiler.enable"):
            ...
            self.funnelSCP.setPortEnabled(SYS_PROFILER_FUNNEL_PORT)
    ...
    def setHaltOnEtfEtrFull(self, halt):
        ...
        if self.sysCTI2.isConnected():
            if halt:
                self.sysCTI2.writeRegister(CTICTRL, 1)
                self.sysCTI2.writeRegister(CTIINEN + ETF_FULL,
                                           1 << CTM_CHAN_SYNC_STOP)
            else:
                self.sysCTI2.writeRegister(CTIINEN + ETF_FULL, 0)
