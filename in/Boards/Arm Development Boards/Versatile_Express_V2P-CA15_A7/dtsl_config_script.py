...
class DtslScript(DTSLv1):
    ...
    def getOptionCortexA7TabPage():
        return DTSLv1.tabPage("cortexA7", "Cortex-A7", childOptions=[
            DTSLv1.booleanOption(
              'coreTrace',
              'Enable Cortex-A7 core trace',
              defaultValue=False,
              childOptions=[
                # Allow each source to be enabled/disabled individually
                DTSLv1.booleanOption(
                  'Cortex_A7_%d' % c,
                  "Enable Cortex-A7 %d trace" % c,
                  defaultValue=True)
                for c in range(0, NUM_CORES_CORTEX_A7)] + [
                DTSLv1.booleanOption(
                  'triggerhalt',
                  "ETM Triggers halt execution",
                  description="Enable the ETM triggers to halt execution",
                  defaultValue=False)] + [
                DTSLv1.booleanOption(
                  'timestamp',
                  "Enable ETM Timestamps",
                  description="Controls the output of timestamps into the ETM output streams",
                  defaultValue=True)] + [
                DTSLv1.booleanOption(
                  'contextIDs',
    ...
    def discoverDevices(self):
        ...
        for i in range(0, NUM_CORES_CORTEX_A15):
            ...
            dev = a15_rams.A15CoreDevice(self, cortexA15coreDev, "Cortex-A15_%d" % i)
            deviceInfo = DeviceInfo("core", "Cortex-A15")
    ...
    def exposeCores(self):
        for core in self.cortexA15cores + self.cortexA7cores:
            self.registerFilters(core)
            self.addDeviceInterface(core)
        for core in self.cortexA15cores:
            a15_rams.registerInternalRAMs(core)
        for core in self.cortexA7cores:
            a7_rams.registerInternalRAMs(core, self.AHB)
    ...
    def setupCTISyncSMP(self):
        ...
        smp = RDDISyncSMPDevice(self, "Cortex-A15 SMP", self.cortexA15cores)
        ...
        smp = RDDISyncSMPDevice(self, "Cortex-A7 SMP", self.cortexA7cores)
        self.registerFilters(smp)
        self.addDeviceInterface(smp)
    ...
    def setupBigLittle(self):
        # big.LITTLE
        cores = [ DeviceCluster("big", self.cortexA15cores), DeviceCluster("LITTLE", self.cortexA7cores) ]
        bigLITTLE = RDDISyncSMPDevice(self, "big.LITTLE", 1, cores)
    ...
    def getCTIForSource(self, source):
        ...
        if source in self.ETMs:
            ...
            if coreNum < len(self.CTIs):
                return (self.CTIs[NUM_CORES_CORTEX_A15+coreNum], 6, CTM_CHANNEL_TRACE_TRIGGER)
    ...
    def getFunnelPortForSource(self, source):
        ...
        for i in range(0, NUM_CORES_CORTEX_A7):
            portMap[self.ETMs[i]] = self.getFunnelPortForCore(NUM_CORES_CORTEX_A15+i)
    ...
    def setClockFrequency(self, freq):
        # This is an early revision of the timestamp generator and the reg map is non-standard
        # CNTFID0 is at  +0x010 not 0x020 as would be expected.
        core0 = (self.cortexA7cores + self.cortexA15cores)[0] # Obtain the first available core
        core0.memWrite(0, 0x2a430010, RDDI_ACC_SIZE.RDDI_ACC_WORD, 0x10000, False, 4, pack('<I', freq))

    def postConnect(self):
        DTSLv1.postConnect(self)

        # set the timestamp generator frequency. The default value in the option is the known value for the board
        ...
        self.setClockFrequency(int(freq))
