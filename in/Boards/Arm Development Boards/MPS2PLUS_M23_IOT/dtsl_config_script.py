...
tmDevs_cortexM23 = ["CSETM"]
coreDevs_cortexM23 = ["Cortex-M23"]
NUM_CORES_CORTEX_M23 = 1
coresDap0 = ["Cortex-M23"]
DSTREAM_PORTWIDTH = 4
...
class DtslScript(DTSLv1):
    ...
    def getOptionList():
        return [
            DTSLv1.tabSet("options", "Options", childOptions=[
                ...
                DtslScript.getOptionCortexM23TabPage()
            ])
        ]
    ...
    @staticmethod
    def getOptionCortexM23TabPage():
        return DTSLv1.tabPage("cortexM23", "Core Trace", childOptions=[
                    DTSLv1.booleanOption('coreTrace', 'Enable Cortex-M23 core trace', defaultValue=False,
                        childOptions =
                            # Allow each source to be enabled/disabled individually
                            [ DTSLv1.booleanOption('Cortex_M23_%d' % core, "Enable " + coreDevs_cortexM23[core] + " trace", defaultValue=True)
                            for core in range(0, NUM_CORES_CORTEX_M23) ] +
    ...
    def discoverDevices(self):
        ...
        for core in range(NUM_CORES_CORTEX_M23):
            # Create core
            coreDevice = Device(self, self.findDevice(coreDevs_cortexM23[core]), coreDevs_cortexM23[core])
            self.cortexM23cores.append(coreDevice)

            # Create Trace Macrocell (if a macrocell exists for this core - disabled by default - will enable with option)
            if not tmDevs_cortexM23[core] == None:
                tm = M_Class_ETMv3_5(self, self.findDevice(tmDevs_cortexM23[core]), streamID, tmDevs_cortexM23[core])
                streamID += 2
                tm.setEnabled(False)
                self.macrocells["cortexM23"].append(tm)
    ...
    def registerTraceSources(self, traceCapture):
        ...
        for core in self.cortexM23cores:
    ...
    def setInitialOptions(self):
        ...
        coreTraceEnabled = self.getOptionValue("options.cortexM23.coreTrace")
        for core in range(NUM_CORES_CORTEX_M23):
            thisCoreTraceEnabled = self.getOptionValue("options.cortexM23.coreTrace.Cortex_M23_%d" % core)
            enableSource = coreTraceEnabled and thisCoreTraceEnabled
            coreTM = self.getTMForCore(self.cortexM23cores[core])
            self.setTraceSourceEnabled(coreTM, enableSource)
            self.setTimestampingEnabled(coreTM, self.getOptionValue("options.cortexM23.coreTrace.timestamp"))
...
class DtslScript_DSTREAM_ST(DtslScript_DSTREAM_ST_Family):
    ...
    def getDSTREAMOptions():
        return (
            ...
            DTSLv1.infoElement(
                "dstream", "", "",
                childOptions=[DTSLv1.enumOption('tpiuPortWidth', 'TPIU port width', defaultValue="4",
