...
class Mutexes( Table ):
    ...
    def readMutex( self, id, members, debugSession ):
        ...
        if isOSRunning( debugSession ):
            ...
            if osEventTbl:
                ...
                for event in events:
                    ...
                    if members[ osEventTypeMember ].readAsNumber( ) == Events.OS_EVENT_TYPE_MUTEX:

                        eventPtr = debugSession.evaluateExpression( "&" + osEventTbl + "[" + str( i ) + "]" )
                        if eventPtr != 0:
                            id = eventPtr.readAsAddress( ).toString( )
                            records.append( self.readMutex( id, members, debugSession ) )
